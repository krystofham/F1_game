...
from init import *
from log import dlog, elog, ilog, snapshot_init_config, wlog
...
def sim_the_lap(
    ...
    if not race_ctx:
        wlog(
            fn="sim_the_lap",
            msg="race_state missing from state.json",
            lap=lap,
            race=race,
        )
    else:
    ...
    # Safety car
    for car in cars:
        SAFETY_CAR, LAPS_REMAINING, car = safety_car(
            car, weather, lap, SAFETY_CAR, LAPS_REMAINING, LAPS
        )
    if SAFETY_CAR:
        LAPS_REMAINING -= 1
    if LAPS_REMAINING == 0:
    ...
    race_ctx = build_race_ctx(
        ...
        wettiness=WETTINESS,
        safety_car=SAFETY_CAR,
        safety_car_laps_remaining=LAPS_REMAINING,
        forecast=forecast,
...
def init_race(
    tracks, race, cars, teams, championship, player, player_2, b, season_count
):
    _cfg = {}
    ...
    try:
        ...
        ilog(
            fn="init_race",
            msg="init.json loaded",
            path=_p,
            config=snapshot_init_config(_cfg, "init_race"),
        )
        ...
        elog(
            fn="init_race",
            msg="init.json malformed, using defaults",
            path=_p,
            error=str(e),
        )
    except OSError as e:
        elog(
            fn="init_race",
            msg="init.json read failed, using defaults",
            path=_p,
            error=str(e),
        )
    ...
    try:
        ...
        wlog(
            fn="init_race",
            msg="climax.json missing/invalid — regenerated defaults",
            path=_climax_path,
            error=str(e),
            climax=climax,
            weather=weather,
    ...
    for car in cars:
        if car.is_player:
            if player.name == car.name:
                ...
                elog(
                    fn="init_race",
                    msg="player car name mismatch",
                    player_1=player.name,
                    player_2=player_2.name,
                    car_name=car.name,
                    car_is_player=car.is_player,
                )
                raise ValueError(
                    "bad condfig in players, contact me on github: https://github.com/krystofham/F1_game/"
                )
    ilog(
        fn="init_race",
    ...
    simulation = qualification(
        simulation, cars, TIME_S1, TIME_S2, TIME_S3, training_type
    )
    ######################################################################################################################################################################
    race_ctx = build_race_ctx(
        weather=weather,
...
def sim_the_race(
    ...
    points, cars, teams, players = plot_graph(
        RANK, teams, cars, player, player_2, climax
    )
