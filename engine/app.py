...
from log import snapshot_state as td_snapshot_state
from models import (
    InitConfigPayload,
    LapUserDataPayload,
    PatchStatePayload,
...
from saving import save_race_csv, save_season_csv, update_track_record
...
def apply_teams_from_state(cars, teams, state_teams, state_drivers=None):
        ...
        if not team:
            ...
            if st_idx < len(teams):
                ...
                wlog(
                    ...
                    msg="team not found and no fallback",
                    team=st.get("name"),
                )
                continue
    ...
    if removed:
        wlog(
            fn="apply_teams_from_state",
            msg="shadow duplicates removed after apply",
            names=removed,
        )

    ilog(
        fn="apply_teams_from_state",
        msg="teams applied ok",
...
def _sync_ai_cars_with_state(cars, state_ai, player_names):
    ...
    for d in unmatched_state:
        if not free_ai_cars:
            elog(
                fn="_sync_ai_cars_with_state",
                msg="no free AI cars left for unmatched state entry",
                missing_name=d["name"],
            )
            break
        ...
        ilog(
            fn="_sync_ai_cars_with_state",
            msg="AI car renamed during sync (transfer side effect)",
            old_name=old_name,
            new_name=d["name"],
        )
...
def load_game_objects(apply_state=True):
    ...
    from init import player as init_player
    from init import player_2 as init_player_2
    ...
    if apply_state:
        ...
        for car, expected_name in slot_map:
            if not expected_name:
                wlog(
                    ...
                    msg="player.name key empty, skipping slot",
                    slot=car.name,
                )
                continue
            ...
            if d:
                ...
                if before != car.name:
                    ilog(
                        fn="load_game_objects",
                        msg="player slot renamed (transfer applied)",
                        slot_init=before,
                ...
                elog(
                    fn="load_game_objects",
                    msg="player not found in state drivers — transfer mismatch?",
                    slot_init=car.name,
                    expected=expected_name,
        ...
        if not teams_ok:
            wlog(
                fn="load_game_objects",
                msg="apply_teams_from_state failed, keeping init.py teams",
    ...
    player = next(
        (c for c in player_cars if c.name == p1_name),
        player_cars[0] if player_cars else None,
    )
    player_2 = next(
...
async def api_init_race():
    ...
    if not track:
        elog(
            fn="api_init_race",
            msg="track not found in database, fallback laps=67 used — THIS IS A BUG",
            race=race,
            fallback_laps=67,
        )
    ...
    ilog(
        fn="api_init_race",
        msg="time_laps reset to [] for new race",
        race=getattr(race, "name", str(race)),
        b=b,
    )
...
async def api_sim_lap():
    ...
    if not race_ctx:
        ...
        raise HTTPException(
            status_code=400, detail="Race not initialized. Call /api/init_race first."
        )
    ...
    if lap > race_ctx["total_laps"]:
        wlog(
            fn="api_sim_lap",
            msg="sim_lap called after race finished",
            lap=lap,
            total=race_ctx["total_laps"],
        )
        raise HTTPException(
            status_code=400, detail="Race already finished. Call /api/post_race."
        )
    ...
    if time_laps is None:
        ...
        elog(
            fn="api_sim_lap",
            msg="time_laps is empty but lap > 0 — data loss suspected",
            lap=lap,
    ...
    if not race:
        wlog(
            fn="api_sim_lap",
            msg="race key missing from state, using fallback",
            fallback="Unknown Race",
        )
        race = "Unknown Race"
    if not k_speed:
        wlog(
            fn="api_sim_lap",
            msg="k_speed missing from race_state, using hardcoded fallback",
        )
    ...
    try:
        lap, cars, teams = sim_the_lap(
            ...
            race_ctx["forecast"][0]
            if len(race_ctx["forecast"]) > 0
            else race_ctx["weather"],
            race_ctx["forecast"][1]
            if len(race_ctx["forecast"]) > 1
            else race_ctx["weather"],
            race_ctx["forecast"][2]
            if len(race_ctx["forecast"]) > 2
            else race_ctx["weather"],
            race_ctx["forecast"][3]
            if len(race_ctx["forecast"]) > 3
            else race_ctx["weather"],
            race_ctx["training_type"],
            race_ctx["k_wear"],
            k_speed,
            race_ctx["speed_bonus"],
            season_count,
            race,
            time_laps,
        )
        ...
        elog(
            fn="api_sim_lap",
            msg="sim_the_lap raised exception",
            error=str(e),
            lap=lap,
            race=race,
        )
    ...
    if len(new_time_laps) == 0 and lap > 0:
        elog(
            fn="api_sim_lap",
            msg="time_laps still empty after sim_the_lap — sim did not write state",
            lap=lap,
            race=race,
        )
    else:
        dlog(
            ...
            msg="lap simmed ok",
            lap=lap,
            time_laps_count=len(new_time_laps),
        )
...
async def api_post_race():
    ...
    if time_laps is None:
        ...
        elog(
            fn="api_post_race",
            msg="time_laps is empty at post_race — race results will be incomplete",
            race=race,
            season=season_count,
            total_laps=race_ctx.get("total_laps"),
            current_lap=state.get("lap"),
        )
    else:
        ilog(
            fn="api_post_race",
            msg="time_laps loaded for post_race",
            race=race,
            time_laps_count=len(time_laps),
        )
    ...
    if "climax" not in race_ctx:
        wlog(
            fn="api_post_race",
            msg="climax missing from race_state, using fallback 'sunny'",
        )

    teams, cars, time_laps = post_race_info(
        time_laps, player, player_2, cars, teams, COUNT_CARS
    )
    ...
    if saved_time_laps == [] and len(time_laps) > 0:
        elog(
            fn="api_post_race",
            msg="time_laps not persisted by save_state_end_of_race — saving bug",
            race=race,
            time_laps_count=len(time_laps),
        )
    else:
    ...
    with open(
        os.path.join(user_input_dir(), "climax.json"), "w", encoding="utf-8"
    ) as f:
        json.dump(
            {"climax": climax, "weather": weather}, f, indent=2, ensure_ascii=False
        )
...
async def api_get_climax():
    ...
    try:
        ...
        elog(
            fn="api_get_climax",
            msg="failed to read climax.json",
            error=str(e),
            path=path,
        )
        return {"climax": "unknown", "weather": "unknown"}
    return {
        "climax": data.get("climax", "unknown"),
...
def api_post_championship():
    ...
    if last_car.is_player:
        ...
        ilog(
            ...
            demoted_player=last_car.name,
            promoted_mmr2=old_best_name,
    ...
    ilog(
        fn="api_post_championship",
        msg="season reset done",
        new_season=season_count,
        p1=player.name,
        p2=player_2.name,
    )
...
async def api_set_init_config(data: InitConfigPayload):
    ...
    try:
        ...
        if written != payload:
            elog(
                fn="api_set_init_config",
                msg="init.json write verify failed — written content differs",
                expected=payload,
                got=written,
            )
        else:
            dlog(
                fn="api_set_init_config",
                msg="init.json written and verified",
                payload=payload,
            )
    except Exception as e:
        elog(
            fn="api_set_init_config",
            msg="init.json verify read failed",
            error=str(e),
            path=path,
        )
...
async def api_get_transfer_offers():
    ...
    offers_pool = [
        {
            ...
            "rating": round(d.get("rating", 1.0), 4),
        }
        for d in ai_drivers
    ]
    ...
    result = {
        "driver_1": {
            "name": players[0]["name"] if len(players) > 0 else "",
            "offers": offers_pool[:5],
        },
        "driver_2": {
            "name": players[1]["name"] if len(players) > 1 else "",
            "offers": offers_pool[5:10],
        },
        "mmr2_best": None,
    }
    ...
    try:
        ...
        elog(
            fn="api_get_transfer_offers",
            msg="MMR2 sim failed, mmr2_best will be null",
            error=str(e),
        )

    ilog(
        fn="api_get_transfer_offers",
        msg="transfer offers generated",
        p1=result["driver_1"]["name"],
        p2=result["driver_2"]["name"],
...
async def api_do_transfer(data: TransferPayload):
    ...
    if not pilot_to_change or not chosen_pilot:
        elog(
            fn="api_do_transfer",
            msg="missing pilot name(s) in request",
    ...
    if pilot_to_change == chosen_pilot:
        elog(
            fn="api_do_transfer",
            msg="pilot_to_change == chosen_pilot, same driver",
    ...
    if pilot_to_change not in (p1_name, p2_name):
        raise HTTPException(
            status_code=400, detail=f"'{pilot_to_change}' is not a player driver"
        )
    ...
    target = next(
        (d for d in drivers if d["name"] == pilot_to_change and d.get("is_player")),
        None,
    )
    if not target:
        elog(
            fn="api_do_transfer",
            msg="target player driver not found in drivers list",
            pilot_to_change=pilot_to_change,
            player_names_in_drivers=[d["name"] for d in drivers if d.get("is_player")],
        )
        raise HTTPException(
            status_code=404, detail=f"Player driver '{pilot_to_change}' not found"
        )
    ...
    if not outgoing_ai:
        elog(
            fn="api_do_transfer",
            msg="chosen AI pilot not found in drivers list",
            chosen_pilot=chosen_pilot,
    ...
    if not old_team_name or not new_team_name:
        wlog(
            fn="api_do_transfer",
            msg="one or both teams are empty string",
            pilot_to_change=pilot_to_change,
    ...
    state["drivers"] = [
        d for d in drivers if not (d["name"] == chosen_pilot and not d.get("is_player"))
    ]
    ...
    ilog(
        ...
        replacement_team=replacement["team"],
...
async def api_sim_race():
    ...
    if time_laps is None:
        elog(
            fn="api_sim_race",
            msg="time_laps key missing from state.json at race start",
            race=race,
            lap=state.get("lap"),
        )
        time_laps = []
        ...
        wlog(
            fn="api_sim_race",
            msg="time_laps not empty at sim_race start — leftover from previous race?",
            race=race,
            time_laps_count=len(time_laps),
        )
    ...
    while lap <= total_laps:
        ...
        if lap > 0 and len(current_time_laps) == 0:
            elog(
                fn="api_sim_race",
                msg="time_laps empty after sim_the_lap — sim not writing state",
                lap=lap,
                race=race,
            )
        ...
        dlog(
            fn="api_sim_race",
            msg="lap simmed",
            smt_occurs=smt_occurs,
            smt_happened=smt_happened,
        )
    ...
    ilog(
        fn="api_sim_race",
        msg="full race sim finished",
        race=race,
        total_laps=total_laps,
        final_time_laps_count=len(final_time_laps),
    )

    if len(final_time_laps) == 0:
        elog(
            fn="api_sim_race",
            msg="time_laps empty after full race — results will be broken at post_race",
            race=race,
            total_laps=total_laps,
        )
...
async def api_sim_until(data: SimUntilPayload):
    ...
    if time_laps is None:
        elog(
            fn="api_sim_until",
            msg="time_laps key missing from state.json",
            race=race,
        ...
        elog(
            fn="api_sim_until",
            msg="time_laps empty but lap > 0 — data loss suspected",
            race=race,
            lap=state.get("lap"),
            target_lap=target_lap,
        )
    ...
    while lap < target_lap and lap <= total_laps:
        ...
        lap, cars, teams = sim_the_lap(
            ...
            current_race_ctx["training_type"],
            current_race_ctx["k_wear"],
            k_speed,
            current_race_ctx["speed_bonus"],
            season_count,
            race,
            time_laps,
        )
        ...
        if lap > 0 and len(current_time_laps) == 0:
            elog(
                fn="api_sim_until",
                msg="time_laps empty after sim_the_lap",
                lap=lap,
                target_lap=target_lap,
                race=race,
            )
...
def get_tracks_api():
    try:
        ...
        dlog(
            fn="load_all_from_json",
            msg="tracks shuffled second time",
            order=[t.name for t in loaded_tracks],
        )
        if not loaded_tracks:
            elog(
                fn="get_tracks_api", msg="Track.load_all_from_json returned empty list"
            )
        ...
        raise HTTPException(status_code=500, detail=f"Interní chyba serveru: {e!s}")
    ...
    try:
        with open(path, mode="r", encoding="utf-8") as f:
            reader = csv.DictReader(
                f
            )  # Má hlavičku: track,driver,team,lap_time,season,race
            for row in reader:
                records.append(row)
    except Exception as e:
