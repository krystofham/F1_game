...
from log import dlog, elog, ilog, snapshot_race_ctx, wlog
...
def _build_standings(cars, teams, time_laps=None):
    ...
    return {
        "drivers": [
            _car_to_dict(c, i + 1, time_laps) for i, c in enumerate(sorted_cars)
        ],
...
def save_state_end_of_lap(
    ...
    dlog(
        fn="save_state_end_of_lap",
        msg="saving lap state",
        lap=lap,
        race=race,
        player_name_arg=player_name,
        player_2_name_arg=player_2_name,
        player_cars_in_memory=player_cars,
        race_ctx=snapshot_race_ctx(race_ctx, "save_state_end_of_lap")
        if race_ctx
        else None,
    )
...
def save_state_end_of_race(cars, teams, season, race, time_laps=None):
    ilog(
        fn="save_state_end_of_race",
        msg="saving end-of-race state",
        race=race,
        season=season,
        time_laps_count=len(time_laps or []),
    )
    data = {
        "type": "race",
        "season": season,
...
def save_season_csv(cars, teams, season_count):
    ...
    drivers_file = os.path.join(
        export_dir, f"season_{season_count}_drivers_{timestamp}.csv"
    )
    sorted_cars = sorted(cars, key=lambda x: x.points, reverse=True)
    ...
    teams_file = os.path.join(
        export_dir, f"season_{season_count}_teams_{timestamp}.csv"
    )
    sorted_teams = sorted(teams, key=lambda x: x.points, reverse=True)
...
def update_track_record(time_laps: list, track: str, season: int) -> bool:
    ...
    if current is None or best_time < float(current["lap_time"]):
        ...
        ilog(
            fn="update_track_record",
            msg="track record broken",
            track=track,
            driver=best_driver,
            lap_time=round(best_time, 4),
            season=season,
            previous=current["lap_time"] if current else "none",
        )
        return True
