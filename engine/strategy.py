...
def strategy(LAPS, TIME_S1, TIME_S2, TIME_S3, tyre, speed, climax):
    ...
    dlog(
        ...
        medium_laps=round(endurance_m, 1),
    ...
    for i, stint in enumerate(wear, 0):
        if count_laps + 4 < stint < count_laps + 20:
            remain = stint - count_laps
            average_speed = (stint - remain) * k_speed[i] / count_laps
    ...
    for i in range(len(wear)):
        if count_laps + 4 < wear[i] + wear[0] < count_laps + 20:
            ...
            average_speed = (
                k_speed[i] * wear[i] + countlapsendurance2 * k_speed[0]
            ) / count_laps
        ...
        if count_laps + 4 < wear[i] + wear[1] < count_laps + 20:
            ...
            average_speed = (
                k_speed[i] * wear[i] + countlapsendurance2 * k_speed[1]
            ) / count_laps
            time = round(((lap_time * LAPS) / average_speed) + box_time, 2)
            if countlapsendurance2 > 0:
                pass
        ...
        if count_laps + 4 < wear[i] + wear[2] < count_laps + 20:
            ...
            average_speed = (
                k_speed[i] * wear[i] + countlapsendurance2 * k_speed[2]
            ) / count_laps
            time = round(((lap_time * LAPS) / average_speed) + box_time, 2)
            if countlapsendurance2 > 0:
                pass
